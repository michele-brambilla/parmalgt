...
struct StapleReKernel {
    ...
    ptSU3 two_by_one(Field_t &U, const Point &n, const Direction &nu) {
        return U[n + mu][mu] * U[n + mu + mu][nu] *
               ...
               U[n + mu][mu] * dag(U[n - nu][mu] * U[n + mu - nu][mu] * U[n + mu + mu - nu][nu]) * U[n - nu][nu] + U[n + mu][nu] * dag(U[n - mu][nu] * U[n - mu + nu][mu] * U[n + nu][mu]) * U[n - mu][mu] + dag(U[n - nu - mu][mu] * U[n - nu][mu] * U[n - nu + mu][nu]) * U[n - nu - mu][nu] * U[n - mu][mu];
    }
    ...
    ptSU3 one_by_one(Field_t &U, const Point &n, const Direction &nu) {
        return U[n + mu][nu] * dag(U[n][nu] * U[n + nu][mu]) + dag(U[n - nu][mu] * U[n + mu - nu][nu]) * U[n - nu][nu];
    }
...
struct GaugeUpdateKernel {
    ...
    GaugeUpdateKernel(const Direction &nu, const double &t) : mu(nu),
#ifndef SF
...
struct WilFlowKernel {
    ...
    WilFlowKernel(const Direction &nu, const double &t) : mu(nu), taug(t) {}
...
struct WilFlowMeasKernel {
    ...
    WilFlowMeasKernel(const Direction &nu, const double &t,
                      Field_t &targ) : mu(nu), taug(t), stau(sqrt(t)), target(&targ) {}
...
struct WilFlowApplyKernel {
    ...
    WilFlowApplyKernel(const Direction &nu, const double &t,
                       Field_t &src) : mu(nu), taug(t), stau(sqrt(t)), source(&src) {}
...
template <class C, class P, class Q>
std::vector<ranlxd::Rand>
...
template <class C>
typename kernels::RSU3Kernel<C>::rand_vec_t
...
class GaugeFixingKernel {
  ...
  private:
    ...
    void do_it(Field_t &U, const Point &n,
        ...
        ptSU3 omega;
        for (Direction mu; mu.is_good(); ++mu)
            omega += dag(U[n - mu][mu].bgf()) * U[n - mu][mu] - U[n][mu] * dag(U[n][mu].bgf());
...
struct ZeroModeSubtractionKernel {
    ...
    ZeroModeSubtractionKernel(const Direction &nu, const ptsu3 &N) : mu(nu), M(exp<BGF, ORD>(-1 * reH(N))) {}
...
struct GammaLowerKernel {
    ...
    void operator()(const Field_t &U, const Point &n) {
        ...
        for (Direction k(1); k.is_good(); ++k) {
            ...
            tmp += (Ctilde * dag(U[n][k]) * dag(U[n - k][k]) * U[n - k][t] * U[n + t - k][k] * U[n + t][k] * dag(U[n + k][t])) * 2. * (3. / 2.) * c[1];
            // the 1x2 contribution, with usual weight c_1
            tmp += (Ctilde * dag(U[n][k]) * U[n][t] * U[n + t][t] * U[n + t + t][k] * dag(U[n + k + t][t]) * dag(U[n + k][t])) * c[1];
        }
#pragma omp critical
        val += tmp;
    }
...
struct GammaUpperKernel {
    ...
    void operator()(const Field_t &U, const Point &n) {
        ...
        for (Direction k(1); k.is_good(); ++k) {
            // the 1x1 contribution
            tmp += (Ctilde * U[n + t][k] * dag(U[n + k][t]) * dag(U[n][k]) * U[n][t]) * c[0];
            // the 2x1 contribution w\ two links @ the boundary
            // we actually have two contributions to the derivative form
            // each boundary link. However, we do not care because the
            // insertion of \tilde C commutes with the links. Hence, we
            // just multiply the weight with two,
            //  w = 2 * (3/2) * c_1 = 3*(-1/12) = -1/4
            tmp += (Ctilde * U[n + t][k] * U[n + t + k][k] * dag(U[n + k + k][t]) * dag(U[n + k][k]) * dag(U[n][k]) * U[n][t]) * 2. * (3. / 2.) * c[1];
            // the 1x2 contribution, with usual weight c_1
            tmp += (Ctilde * U[n + t][k] * dag(U[n + k][t]) * dag(U[n + k - t][t]) * dag(U[n - t][k]) * U[n - t][t] * U[n][t]) * c[1];
        }
#pragma omp critical
        val += tmp;
    }
...
struct WilsonTreeLevelKernel : public base_types<Field_t> {
  public:
    ...
    WilsonTreeLevelKernel(const Field_t &G, const FermionField &X,
                          const double &m) : U(G), src(X), mass(m){};
