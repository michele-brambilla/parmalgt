...
int initialize(int argc, char** argv){
  ...
  sprintf(nspt_pars.confn,"%s%s_nf0_%dx%dx%dx%d_a%2.2f_tg%3.3f_o%d.r%d.dat",
	  ...
	  act_pars.sz[2],act_pars.sz[3],fabs(act_pars.alpha), act_pars.tau_g, allocORD,rank);
  ...
  sprintf(nspt_pars.confn,"%s%s_nf0_%dx%dx%dx%d_a%2.2f_tg%3.3f_o%d.dat",
	  nspt_pars.confn, type_g, act_pars.sz[0], act_pars.sz[1], 
	  act_pars.sz[2],act_pars.sz[3],fabs(act_pars.alpha), act_pars.tau_g, allocORD);
...
#ifdef  __WILLOOP_2x2__
	  << "Placchetta 2x2\t" << nspt_pars.name2x2 << std::endl
#endif
...
int QuenchedAllocate(ptBoson_fld& Umu){
...
int AggiornaParametri(ptBoson_fld& Umu) {
...
int NsptFinalize(ptBoson_fld& Umu, int t){
